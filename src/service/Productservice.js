...
export const uploadImageProduct = async (formData) => {
    try {
        const response = await axios.post('http://localhost:2000/product/upload', formData, {
            headers: {
                'Content-Type': 'multipart/form-data', // Dùng đúng Content-Type khi gửi FormData
            },
        });
        console.log(response.data);
        return response.data; // Trả về URL hình ảnh đã tải lên
    } catch (error) {
